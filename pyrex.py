...
def main():
    def capture(args):
        ...
        if not conffile:
            ...
            try:
                ...
                sys.stderr.write(
                    "Cannot find pyrex conf version! Restoring from template\n"
                )
        ...
        build_config["build"]["initcommand"] = " ".join(
            shlex.quote(a) for a in [oeinit] + args.init
        )
    ...
    def build(args):
        ...
        if use_docker(config):
            ...
            if (
                provider == "docker"
                and int(version.split(".")[0]) < MINIMUM_DOCKER_VERSION
            ):
                sys.stderr.write(
            ...
            if config["config"]["buildlocal"] == "1":
                if VERSION_TAG_REGEX.match(tag.split(":")[-1]) is not None:
                    ...
                    sys.stderr.write(
                        "Try changing 'config:pyrextag' to a different value\n"
                    )
                    return 1
                ...
                if config["config"]["registry"]:
                    docker_args.extend(
                        [
                            "--build-arg",
                            "MY_REGISTRY=%s/" % config["config"]["registry"],
                        ]
                    )
                ...
                try:
                    if os.environ.get(
                        "PYREX_DOCKER_BUILD_QUIET", "1"
                    ) == "1" and config["dockerbuild"].getboolean("quiet"):
            ...
            print(
                textwrap.fill(
                    "Running outside of container. No guarantees are made about your Linux "
        ...
        with open(runfile, "w") as f:
            f.write(
                ...
                """.format(
                        pyrexroot=config["build"]["pyrexroot"],
        ...
        with open(shellfile, "w") as f:
            f.write(
                ...
                exec {runfile} {shell} "$@"
                """.format(
                        runfile=runfile, shell=config["config"]["shell"]
                    )
                )
            )
            ...
            for g in globs:
                for cmd in glob.iglob(g):
                    ...
                    if (
                        ...
                        os.symlink(
                            target.format(command=cmd), os.path.join(shimdir, name)
                        )
    ...
    def run(args):
        ...
        if use_docker(config):
            ...
            init_command = config["run"].get(
                "initcommand", config["build"]["initcommand"]
            )
